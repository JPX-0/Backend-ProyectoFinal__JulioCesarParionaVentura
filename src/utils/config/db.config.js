...
module.exports = {
  mongodb: {
    ...
    simulateConnection: database => {
      if(database) return `mongodb+srv://${env.DB_MAIL}:<PASSWORD>@${env.DB_DEPLOY}.mongodb.net/${database}?retryWrites=true&w=majority`
      return `mongodb+srv://${env.DB_MAIL}:<PASSWORD>@${env.DB_DEPLOY}.mongodb.net/ecommerce?retryWrites=true&w=majority`
    }
  },
  firebase: {
    connect: () => path.resolve(cwd() + "/models/daos/cloud/firebase/TOKEN/firebase.token.json")
  },
  file: {
    connect: async() => {
      try {

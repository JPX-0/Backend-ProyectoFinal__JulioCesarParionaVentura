const codeInfo = new (require("../../../../md/app.info"))().services.user;
...
class UserServices {
  ...
  async login({ email, password }) {
    ...
    if (!user || !(await compareEncrypted(password, user.password))) return { user: {}, error: true };
    return { user, error: false };
  }

  async signup(email, { password, repeatPassword }, user, createCartTogetherWithUser) {
    /************************/
    /****** Learn More ******/
    /**/ codeInfo.signup  /**/
    /************************/
    ...
    let createdCart;
    try {
      const creatingUser = this.dto.generate.user({ email, password: await encrypt(password) }, user);
      createdUser = await this.repo.user.create(this.dto.generic.create(creatingUser));
      if(!createCartTogetherWithUser) {
        const creatingCart = this.dto.generate.cart({ idUser: createdUser._id, address: createdUser.address });
        createdCart = await this.repo.cart.create(this.dto.generic.create(creatingCart));
      }
      return { user: createdUser, cart: createdCart, error: false };
    } catch (error) { return { user: (createdUser || {}), cart: (createdCart || {}), error: true } }
  }

  async unregistered(idUser, idCart) {
    /******************************/
    /********* Learn More *********/
    /**/ codeInfo.unregistered  /**/
    /******************************/
    if(idUser) {
  ...
  async update(idUser, user) {
    ...
    const updatingUser = async (dataToUpdate, updater) => {
      ...
      if(updater.email) {
        const allUserOrders = await this.repo.order.get({ email: dataToUpdate.email, all: true });
      ...
      if(updater.password) dataToUpdate.password = await encrypt(updater.password);
    ...
    const updatedUser = await updatingUser(userFound, user);
		const message = await this.repo.user.update(idUser, this.dto.generic.update(updatedUser));
    return message;
  }
}

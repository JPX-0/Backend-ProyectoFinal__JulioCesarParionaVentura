...
const renderProfile = async (req, res, next) => {
  ...
  try {
    const user = req.user;
    res.render("ejs/index", template.render("Mi Perfil", { renderProfile: { user } }));
  } catch (error) { next(error); }
}
...
const renderProfilePicture = async (req, res, next) => {
  ...
  try {
    ...
    res.render("ejs/index", template.render("Ver mi avatar", { renderProfilePicture: { ...renderImg } }));
  } catch (error) { next(error); }
}
...
module.exports = { renderSessions, renderProfile, renderProfilePicture, renderLogout, renderError };
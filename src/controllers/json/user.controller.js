    ...
    try {
      if(idUser) {
        ...
        if(userFound._id.toString() == req.user?._id.toString()) 
          ...
          return res.status(NOT_AUTHORIZED.code).json(apiResponse(message, NOT_AUTHORIZED.code));
        }
      }
  ...
  async postUser(req, res, next) {
    ...
    try {
      ...
      if(error) return res
        ...
        .json(apiResponse("An error occurred while creating the user.", BAD_REQUEST.code));
      await sendEmail({ user }, `Nuevo registro`);
      if(!env.CREATE_CART_TOGETHER_WITH_USER) {
        const response = { copyIdUser: user._id }
        return res.status(CREATED.code).json(apiResponse(response, CREATED.code));
      }
